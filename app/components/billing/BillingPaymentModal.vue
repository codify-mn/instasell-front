        ...
        <template #content>
            <div class="p-6">
                ...
                <div class="flex items-center justify-between mb-5">
                    <div>
                        <h3 class="text-base font-bold text-[var(--text-heading)]">Төлбөр төлөх</h3>
                        <p v-if="invoice?.plan" class="text-sm text-[var(--text-muted)]">{{ invoice.plan.name }}</p>
                    </div>
                ...
                <div v-if="invoice?.qpay_data" class="space-y-5">
                    ...
                    <div class="text-center py-4 bg-[var(--surface-inset)] rounded-lg">
                        <p class="text-xs text-[var(--text-muted)] mb-1">Нийт төлбөр</p>
                        <p class="text-2xl font-bold text-[var(--text-heading)]">{{ formatAmount(invoice.amount) }}</p>
                    ...
                    <div class="flex flex-col items-center gap-3">
                        <div class="p-3 bg-white rounded-xl border border-[var(--border-primary)]">
                            ...
                            <div v-else class="w-44 h-44 flex items-center justify-center">
                                <UIcon name="i-lucide-qr-code" class="size-12 text-[var(--text-placeholder)]" />
                            </div>
                        </div>
                        <p class="text-xs text-[var(--text-muted)]">QR кодыг банкны апп-аар уншуулна уу</p>
                    </div>

                    <!-- Bank deep links -->
                    <div v-if="invoice.qpay_data.deep_links?.length">
                        <p class="text-xs font-semibold text-[var(--text-heading)] mb-2">Банкны апп-аар нээх</p>
                        <div class="grid grid-cols-4 gap-2">
                            <a
                                ...
                                :href="link.link"
                                target="_blank"
                                class="flex flex-col items-center gap-1 p-2 rounded-lg hover:bg-[var(--surface-inset)] transition-colors"
                            >
                                <img v-if="link.logo" :src="link.logo" :alt="link.name" class="w-9 h-9 rounded-lg object-cover" />
                                <span class="text-[10px] text-[var(--text-muted)] text-center line-clamp-1">{{ link.name }}</span>
                            </a>
                        </div>
                    </div>
                ...
                <div v-else class="py-12 text-center">
                    <UIcon name="i-lucide-loader-2" class="size-6 animate-spin text-[var(--text-muted)] mx-auto mb-2" />
                    <p class="text-sm text-[var(--text-muted)]">Нэхэмжлэх үүсгэж байна...</p>
                </div>
            </div>
        </template>
    </UModal>
