...
const { value, next } = useCycle([0, 10, 909])
useRootClick(next)
</script>

<template>
    <DashboardCard padding="md" hover>
        <div class="space-y-3">
            <div class="flex items-center justify-between">
                <p
                    class="text-xs truncate font-semibold text-(--text-muted) uppercase tracking-wider"
                >
                    {{ label }}
                </p>
                <div
                ...
                >
                    <UIcon :name="icon" class="w-5 h-5 m-2" />
                </div>
            </div>

            <div class="flex items-end gap-2">
                <span class="text-3xl font-bold text-(--text-heading)">
                    <NumberFlow
