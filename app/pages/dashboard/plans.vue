        ...
        <div class="flex-1 overflow-y-auto">
            <div class="max-w-5xl mx-auto px-4 sm:px-7 py-6">
                ...
                <div class="flex items-center justify-center mb-8">
                    <div class="inline-flex items-center gap-1 p-1 rounded-lg border border-(--border-primary) bg-(--surface-card)">
                        <button
                ...
                <div v-if="loading" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                    <div v-for="i in 4" :key="i" class="rounded-xl border border-(--border-primary) bg-(--surface-card) p-5 space-y-4">
                        <div class="space-y-2">
                            <div class="h-4 w-20 rounded bg-(--surface-inset) animate-pulse" />
                            <div class="h-3 w-32 rounded bg-(--surface-inset) animate-pulse" />
                        </div>
                        <div class="h-7 w-28 rounded bg-(--surface-inset) animate-pulse" />
                        <div class="space-y-2">
                            <div v-for="j in 6" :key="j" class="h-3 rounded bg-(--surface-inset) animate-pulse" :class="j % 2 === 0 ? 'w-3/4' : 'w-full'" />
                        </div>
                        <div class="h-9 w-full rounded-md bg-(--surface-inset) animate-pulse" />
                    </div>
                </div>
                ...
                <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                    <div
                        v-for="plan in plans"
                        :key="plan.slug"
                        class="relative flex flex-col rounded-xl border bg-(--surface-card) transition-all duration-200"
                        :class="[
                        ...
                        <div class="p-5 flex-1 flex flex-col">
                            ...
                            <div class="mb-4">
                                <div class="flex items-center justify-between mb-1">
                                    <h3 class="text-base font-bold text-(--text-heading)">{{ plan.name }}</h3>
                                    <span v-if="isCurrentPlan(plan)" class="text-xs font-semibold text-primary-500">Одоогийн</span>
                                </div>
                                <p class="text-xs text-(--text-muted)">{{ plan.description }}</p>
                            </div>

                            <!-- Price -->
                            <div class="mb-5">
                                <div class="flex items-baseline gap-0.5">
                                    <span class="text-2xl font-extrabold text-(--text-heading)">
                                        {{ formatPrice(isYearly ? Math.round(plan.yearly_price / 12) : plan.monthly_price) }}₮
                                    </span>
                                    <span class="text-sm text-(--text-muted)">/сар</span>
                                </div>
                                <div v-if="isYearly && plan.monthly_price > 0" class="flex items-center gap-1.5 mt-1">
                                    <span class="text-xs text-(--text-placeholder) line-through">{{ formatPrice(plan.monthly_price * 12) }}₮/жил</span>
                            ...
                            <p v-if="plan.trial_days > 0 && !isCurrentPlan(plan)" class="text-center text-xs text-(--text-muted) mt-2">
